...
const MOCK_RATES: Record<string, Record<string, number>> = {
  USD: { EUR: 0.92, GBP: 0.79, BDT: 110.5, NGN: 1580.0, USD: 1.0 },
...
export class FxService {
  ...
  private fetchLiveRate(base: string, quote: string): number {
    ...
    if (!rate) {
      throw new BadRequestException(
        `Unsupported currency pair: ${base}/${quote}`,
      );
    }
    return rate;
  }
  ...
  async getQuote(
    quoteId: string,
  ): Promise<FxQuote & { secondsRemaining: number }> {
  ...
  async consumeQuote(quoteId: string, transactionId: string): Promise<FxQuote> {
    ...
    this.logger.log(
      `FX quote consumed: id=${quoteId} by txn=${transactionId} rate=${quote.rate}`,
    );
    return quote;
  }
}
