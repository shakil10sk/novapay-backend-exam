...
export class TransactionService {
  ...
  async executeTransfer(dto: CreateTransferDto): Promise<any> {
    ...
    let idempKey = await this.idempotencyRepo.findOne({
    ...
    if (idempKey) {
      ...
      if (new Date() > idempKey.expiresAt) {
        this.logger.log(
          `Idempotency key expired: ${dto.idempotencyKey}. Treating as new.`,
        );
        ...
        if (idempKey.payloadHash !== payloadHash) {
          throw new UnprocessableEntityException(
            'Idempotency key payload mismatch. This key was already used for a different request.',
          );
        }
        ...
        if (idempKey.status === IdempotencyStatus.COMPLETED) {
          this.metricsService.idempotencyHits.inc();
          this.logger.log(
            `Duplicate request detected: ${dto.idempotencyKey}. Returning cached response.`,
          );
    ...
    this.metricsService.transactionTotal.inc({
    ...
    try {
      ...
      await this.idempotencyRepo.save(idempKey);
      this.metricsService.transactionFailed.inc({ reason: e.message });
      throw err;
    }
  }
   ...
   */
  private async orchestrateTransaction(
    dto: CreateTransferDto,
  ): Promise<Transaction> {
    const sender = await this.accountService.getWalletById(dto.senderWalletId);
    const receiver = await this.accountService.getWalletById(
    ...
    if (dto.fxQuoteId) {
      ...
      if (
        quotePreview.baseCurrency !== sender.currency ||
        quotePreview.quoteCurrency !== receiver.currency
      ) {
        throw new BadRequestException(
          'FX quote currency mismatch with wallet currencies.',
        );
      }
      // fxRate/creditAmount will be set once the quote is consumed (after txn persisted)
    } else if (sender.currency !== receiver.currency) {
      throw new BadRequestException(
        'FX quote ID is required for cross-currency transfers.',
      );
    }
    ...
    try {
      ...
      if (dto.fxQuoteId) {
        const consumed = await this.fxService.consumeQuote(
          dto.fxQuoteId,
          savedTxn.id,
        );
      ...
      this.accountService
        .updateBalanceSnapshot(
          sender.id,
          String(senderBalance.balance - dto.amount),
        )
        .catch(() => {});
      this.ledgerService
        .getBalance(receiver.id)
        .then((b) =>
          this.accountService.updateBalanceSnapshot(
            receiver.id,
            String(b.balance),
          ),
        )
      ...
      const e = err as Error & { message?: string };
      this.logger.error(
        `Transaction orchestration failed for txn=${savedTxn.id}: ${e.message}`,
      );
      savedTxn.status = TransactionStatus.FAILED;
      savedTxn.failureReason = e.message;
      await this.transactionRepo.save(savedTxn);
      throw err;
    }
  }
  ...
  async runCrashRecovery(): Promise<void> {
    ...
    for (const txn of pendingTransactions) {
      ...
      if (entries.length === 2) {
        ...
        this.logger.log(
          `Recovered: Set txn=${txn.id} to COMPLETED (found ledger entries).`,
        );
      } else {
        // Incomplete or no entries (Atomicity failure) -> Mark as failed to allow user retry
        txn.status = TransactionStatus.FAILED;
        txn.failureReason =
          'Server crash detected during execution. Entries incomplete.';
        await this.transactionRepo.save(txn);
        this.logger.log(
          `Recovered: Set txn=${txn.id} to FAILED (ledger entries missing).`,
        );
      }
    }
  }
